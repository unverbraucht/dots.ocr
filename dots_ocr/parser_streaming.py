...
class DotsOCRParser:
    ...
    def parse_pdf(self, input_path, filename, prompt_mode, save_dir):
        ...
        def _execute_task(page_idx):
            # convert a single page to image
            origin_image = get_image_by_fitz_doc(doc[page_idx], target_dpi=self.dpi)
        ...
        results = []
        next_page = 0
        max_inflight = num_thread * 2

        with ThreadPoolExecutor(max_workers=num_thread) as executor:
            futures = {}

            # Prime the pool with up to max_inflight tasks
            while next_page < total_pages and len(futures) < max_inflight:
                fut = executor.submit(_execute_task, next_page)
                futures[fut] = next_page
                next_page += 1

            with tqdm(total=total_pages, desc="Processing PDF pages") as pbar:
                while futures:
                    for fut in as_completed(futures):
                        page_idx = futures.pop(fut)
                        try:
                            result = fut.result()
                        ...
                        # As soon as one finishes, submit the next page (if any remain)
                        if next_page < total_pages:
                            new_fut = executor.submit(_execute_task, next_page)
                            futures[new_fut] = next_page
                            next_page += 1

                        break  # go back to wait for next completed future
